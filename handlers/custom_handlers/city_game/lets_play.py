...
async def lest_play(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    """
    await callback.message.reply('Давай сыграем в города\n'
    ...
    cities = list()
    path = os.path.abspath(os.path.join('handlers', 'custom_handlers',
    ...
    if game_start[-1] != 'ь' and game_start[-1] != 'ъ':
        await callback.message.answer(f'Вам на букву {game_start[-1]}',
                                      reply_markup=kb_game())
    else:
        await callback.message.answer(f'Вам на букву {game_start[-2]}',
                                      reply_markup=kb_game())
